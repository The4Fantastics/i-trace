...
public class MOFScript2iTrace_Shortcut implements ILaunchShortcut {
	...
	protected void searchAndLaunch(String pathIN, String mode) {		
		...
		try {
			...
			if (previousCount != newCount) {	// Si hemos creado una nueva configuracion
				...
				DebugUITools.openLaunchConfigurationDialog(getShell(), 
						config, DebugUITools.getLaunchGroup(config, mode).getIdentifier(), null);
			}
